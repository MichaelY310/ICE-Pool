...
def prepare_val_data(graphs, args, val_idx, max_nodes=0):
    ...
    if not os.path.exists(f"preprocessed_data/{args.bmname}.pt"):
        ...
        torch.save([all_assign_matrices, all_edgefeat_lists, all_edgepool_matrices_dics, all_num_nodes_after_det_pool], f"preprocessed_data/{args.bmname}.pt")
        print("Deterministic pooling completed.")

    else:
        all_assign_matrices, all_edgefeat_lists, all_edgepool_matrices_dics, all_num_nodes_after_det_pool = torch.load(f"preprocessed_data/{args.bmname}.pt")
