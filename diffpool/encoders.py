...
class DET_SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, max_num_clusters_fordet, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                 assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1,
                 pred_hidden_dims=[50], concat=True, bn=True, dropout=0.0, linkpred=True,
                 assign_input_dim=-1, device="cuda", args=None):
        '''
...
class ICE_SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                 assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1,
                 pred_hidden_dims=[50], concat=True, bn=True, dropout=0.0, linkpred=True,
                 assign_input_dim=-1, use_simple_gat=False, egat_hidden_dims=[128, 128], egat_dropout=0.6, egat_alpha=0.2, egat_num_heads=[1, 3], pooling_size_real=8, DSN=False, device="cuda", args=None):
        '''
        Args:
            num_layers: number of gc layers before each pooling
            num_nodes: number of nodes for each graph in batch
            ...
            if i == 0:
                ...
                if use_simple_gat:
                    self.egat_list.append(
                        BatchGraphAttentionLayer((self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, embedding_dim, self.egat_dropout, self.egat_alpha).to(
                            device))
                else:
                    self.egat_list.append(
                        BatchGATFeatureExtractor((self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, self.edgefeat_dim, self.egat_hidden_dims, embedding_dim, self.egat_dropout, self.egat_alpha, self.egat_num_heads).to(
                            device))
            else:
                if use_simple_gat:
                    self.egat_list.append(
                        BatchGraphAttentionLayer(self.pred_input_dim * 1, embedding_dim, self.egat_dropout, self.egat_alpha).to(
                            device))
                else:
                    self.egat_list.append(
                        BatchGATFeatureExtractor(self.pred_input_dim * 1, self.edgefeat_dim, self.egat_hidden_dims, embedding_dim, self.egat_dropout, self.egat_alpha, self.egat_num_heads).to(
                            device))
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        out, _ = torch.max(embedding_tensor, dim=1)  # Sum over nodes: batch_size * [(num_layers-1) * hidden_size + embedding_size]
        out_all.append(out)
        if self.num_aggs == 2:
            out = torch.sum(embedding_tensor, dim=1)
            out_all.append(out)
        ...
        for i in range(self.num_pooling):
            ...
            self.assign_prob = torch.nn.Softmax(2)(self.assign_tensor * max_num_nodes) * embedding_mask * assign_mask.transpose(-1, -2)
    ...
    def connection_entropy(self, A, S):
        A_ext = A - (S @ S.T) * A
        # S @ S.T is a mask such that if node i and node j are in the same cluster, then [i, j] = 1
        # Applying this mask on A gives us A_int, the internal connectivity matrix within clusters
        # A - A_int  =  A_ext
        SA = S.T @ A_ext  # SA[i, j] is the number of edges from node i to cluster j
...
class ICE_DET_SoftPoolingGcnEncoder(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes, det_assign_matrix, det_edgefeat_list, batch_num_nodes_after_det_pool, **kwargs):
        if 'assign_x' in kwargs:
            x_a = kwargs['assign_x']
        else:
            x_a = x
        ...
        for i in range(self.num_pooling):
            ...
            assign_mask = self.construct_mask(self.assign_prob.shape[-1], batch_num_nodes_after_det_pool)
...
class SVD_DET_SoftPoolingGcnEncoder(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes, det_assign_matrix, det_edgepool_matrices_dic, batch_num_nodes_after_det_pool, **kwargs):
        if 'assign_x' in kwargs:
            x_a = kwargs['assign_x']
        else:
            x_a = x
        ...
        for i in range(self.num_pooling):
            pool = Pool_Edges(det_edgepool_matrices_dic,
                              ...
                              device=self.device)
            self.assign_tensor = det_assign_matrix
            assign_mask = self.construct_mask(self.assign_tensor.shape[-1], batch_num_nodes_after_det_pool)
            ...
            embedding_tensor = self.gcn_forward(x, adj,
                                                self.conv_first_after_pool[i], self.conv_block_after_pool[i],
                                                self.conv_last_after_pool[i])
            embedding_tensor = embedding_tensor * assign_mask
...
class ICE_SVD_DET_SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, max_num_clusters_fordet, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        for i in range(num_pooling):
            ...
            if i == 0:
                ...
                if use_simple_gat:
                    self.egat_list.append(
                        BatchGraphAttentionLayer(mul*(self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, embedding_dim, self.egat_dropout, self.egat_alpha).to(
                            device))
                else:
                    self.egat_list.append(
                        BatchGATFeatureExtractor(mul*(self.hidden_dim * (num_layers - 1) + embedding_dim) * 1, self.edgefeat_dim, self.egat_hidden_dims, embedding_dim, self.egat_dropout, self.egat_alpha, self.egat_num_heads).to(
                            device))
            else:
                if use_simple_gat:
                    self.egat_list.append(
    ...
    def forward(self, x, adj, batch_num_nodes, det_assign_matrix, det_edgefeat_list, det_edgepool_matrices_dic, batch_num_nodes_after_det_pool, **kwargs):
        if 'assign_x' in kwargs:
            x_a = kwargs['assign_x']
        else:
            x_a = x
        ...
        for i in range(self.num_pooling):
            ...
            if self.DSN:
                edgefeat = batch_DSN_fast(edgefeat)
            edgefeat_mask = (adj != 0).float()
            embedding_tensor = self.egat_forward(self.egat_list[i], x, edgefeat, assign_mask, edgefeat_mask)
