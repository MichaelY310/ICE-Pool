...
def benchmark_task_val(args, feat='node-label', pred_hidden_dims = [50], device='cuda', log_dir=None):
    ...
    if args.use_SVD:
        data_out_dir = 'data/data_preprocessed_withSVD/' + args.bmname + '/pool_sizes_' + args.pool_sizes
    if args.normalize ==0:
    ...
    for i in range(NUM_TESTS):
        ...
        if args.with_test:
            _, train_accs, val_accs, test_accs, best_val_result = train(train_dataset, model, args, val_dataset=val_dataset, test_dataset=test_dataset,
             log_dir = log_out_file, device=device)
        else:
            _, train_accs, val_accs, test_accs, best_val_result = train(train_dataset, model, args, val_dataset=val_dataset, test_dataset=None,
             log_dir = log_out_file, device=device)
        ...
        all_results.append({"train_accs": train_accs, "val_accs": val_accs, "test_accs": test_accs, "best_val_result": best_val_result["acc"], "args": args})
...
def main():
    ...
    log_dir = f"{prog_args.bmname}_edgefeat{prog_args.use_edgefeat}_DSN{prog_args.DSN}_lr{prog_args.lr}_"
    if prog_args.use_SVD:
        log_dir = f"{prog_args.bmname}_edgefeat{prog_args.use_edgefeat}_DSN{prog_args.DSN}_lr{prog_args.lr}_SVD_"
    if prog_args.SVD_only:
        log_dir = f"{prog_args.bmname}_edgefeat{prog_args.use_edgefeat}_DSN{prog_args.DSN}_lr{prog_args.lr}_SVDonly_"
